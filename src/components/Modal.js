...
const Modal = ({
  ...
  const [token0, setToken0] = useState(
    new Token(4, pool.ids.from, 18, pool.symbols.from)
  );
  const [token1, setToken1] = useState(
    new Token(4, pool.ids.to, 18, pool.symbols.to)
  );
  ...
  const getMidPrice = async (pool) => {
    ...
    const balance0 = parseFloat(
      ethers.utils.formatUnits(balance0Obj.toString(), token0.decimals)
    ).toFixed(3);
    const balance1Obj = await token1Contract.balanceOf(address);
    const balance1 = parseFloat(
      ethers.utils.formatUnits(balance1Obj.toString(), token1.decimals)
    ).toFixed(3);
    let allowance0, allowance1;

    if (token0.symbol !== "WETH") {
      const allowance0Obj = await token0Contract.allowance(
        address,
        routerAddress
      );
    ...
    if (token1.symbol !== "WETH") {
      const allowance1Obj = await token1Contract.allowance(
        address,
        routerAddress
      );
    ...
    const liquidity0 = parseFloat(
      ethers.utils.formatUnits(
        pair.reserveOf(token0).raw.toString(),
        token0.decimals
      )
    );
    const liquidity1 = parseFloat(
      ethers.utils.formatUnits(
        pair.reserveOf(token1).raw.toString(),
        token1.decimals
      )
    );
      ...
      <div className="modal__dialog" onClick={handleModalDialogClick}>
        ...
        <div className="exchange">
          ...
          {needFromAuth || needToAuth ? (
            ...
            <div>
              <Confirm
                onClick={(e) => console.log("This function will add liquidity")}
              />
            </div>
          )}
